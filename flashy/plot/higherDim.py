...
def flashSpeeds(fname, thresh=1e-6, filetag='speeds', batch=False,
    ...
    """
    Plot species distribution in 'speed' space
    for a set wedge in the domain. ATM the wedge is
    fixed at a polar angle of 0-45 degrees at
    90 degrees azimuth.
    ...
    """
    ds = yt.load(fname)
    box = ds.box(ds.arr([-3e9, 0.0, -1e8], "code_length"),
                 ds.domain_right_edge)
    # left edge is extended in -z to include the central cell
    cylinder = ds.disk(center=ds.arr([0.0, -3.0e9, 3.0e9], "code_length"),
                       normal=ds.arr([0.0, -1.0/1.414213562, 1.0/1.414213562],
                                     "unitary"),
                       radius=(1e11, "code_length"),
                       #  height=(3e9*1.414213562, "code_length"))
                       # this is slightly larger (0.7%) to include edge cells
                       height=(3e9*1.424213562, "code_length"))
...
#         masses = \
#         [rawd[j][i]*rawd[3][i] for j in range(offset,len(species)+offset)]
...
# legacy funcs
def bifold(fname, mhead=True, topfield='density', tlims=[1e-1, 4e7],
           width=1.1e9, botfield='temperature', blims=[3e7, 2e9],
           lintop=False,  batch=False, linbot=False, name='', cmap=''):
    ...
    plotFRB(grid[1], grid.cbar_axes[1],
            ...
            blims, top=False, linear=linbot, cmap=cmap)
    if mhead:
...
def plotFRB(gridAx, cbgAx, imArr, lims, top=True, linear=False, cmap=''):
    ...
    if top:
        im = gridAx.imshow(imArr,
                           ...
                           # horizontal mi/ma vertical mi/ma
                           extent=[yl, yr, xl, xr],
                           norm=norm)
    ...
    # yt flips axes, plus imarray is transposed,
    # this must be an hdf5 saving issue
    ...
    unt = str(unit).replace('**', '^')
    label = '{} ($10^{{{}}} {}$)'.format(field.capitalize(), scaling, unt)
    cax.set_label_text(label, fontsize=11.0)
