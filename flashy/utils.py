...
def get_bearing(angles, geom):
    ...
    Args:
        angles(float list): empty for 1d. angle from equator for 2d.
            spherical angles for 3d (polar angle and azimuth).
        polar and azimuthal angle (empty list for 1D).
        geom(str): geometry string (cartesian, spherical, cylindrical).

    Returns:
        (str, np.array): parsed direction name, cartesian direction.
    ...
    if len(angles) == 0:
        ...
        angles = np.radians(angles)
        gam = np.cos(angles[0])
        x = gam*np.cos(angles[1])
        y = gam*np.sin(angles[1])
        z = np.sin(angles[0])
        return 'radius', np.array([np.round(x), np.round(y), np.round(z)])
    else:
